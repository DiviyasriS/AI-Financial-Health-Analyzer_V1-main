...
return Ok(dto);
    ...
    public async Task<IActionResult> GetInsights()
    {
        ...
        // Always calculate the current risk level from the same transaction snapshot.
        // Do not depend on an older saved risk row or default to Low.
        (string riskLevel, _) = _riskPredictionService.Predict(features);
    ...
    private async Task<RiskDto> BuildAndPersistRiskDtoAsync(int userId)
    {
        SpendingSummaryDto summary = await _transactionService.GetSummaryAsync(userId);
        ...
        if (summary.TotalTransactions == 0)
        {
            return new RiskDto
            {
                ...
                RiskFactors = new List<string> { "No transactions available for analysis." },
